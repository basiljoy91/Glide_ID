...
func (s *AuditService) ListSuperAdminLogs(ctx context.Context, filters AuditLogFilters) ([]AuditLogEntry, int, error) {
	limit := filters.Limit
	if limit <= 0 {
		limit = 50
	}
	...
	where := []string{"(COALESCE(actor.role::text, '') = 'super_admin' OR al.details->>'scope' = 'super_admin')"}
	...
	if strings.TrimSpace(filters.TenantID) != "" {
		...
		where = append(where, fmt.Sprintf("al.tenant_id = $%d", len(args)))
	}
	if q := strings.TrimSpace(filters.Query); q != "" {
		args = append(args, "%"+strings.ToLower(q)+"%")
		where = append(where, fmt.Sprintf(`(
			...
			LOWER(COALESCE(target.first_name || ' ' || target.last_name, '')) LIKE $%d OR
			LOWER(COALESCE(target.email, '')) LIKE $%d OR
			LOWER(COALESCE(tenant.name, '')) LIKE $%d OR
			LOWER(al.action::text) LIKE $%d OR
			LOWER(COALESCE(al.resource_type, '')) LIKE $%d
	...
	listQuery := fmt.Sprintf(`
		SELECT
			al.id, al.tenant_id, tenant.name, al.user_id,
			CASE WHEN actor.id IS NULL THEN NULL ELSE actor.first_name || ' ' || actor.last_name END AS actor_name,
			actor.email,
			al.target_user_id,
			CASE WHEN target.id IS NULL THEN NULL ELSE target.first_name || ' ' || target.last_name END AS target_name,
	...
	for rows.Next() {
		...
		var detailsRaw []byte
		if err := rows.Scan(
			&entry.ID, &entry.TenantID, &entry.TenantName, &entry.UserID, &entry.ActorName, &entry.ActorEmail,
			&entry.TargetUserID, &entry.TargetName, &entry.TargetEmail,
			&entry.Action, &entry.ResourceType, &entry.ResourceID, &detailsRaw,
			&entry.IPAddress, &entry.UserAgent, &entry.CreatedAt,
		); err != nil {
