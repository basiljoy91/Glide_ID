...
func ListSuperAdminAuditLogs(auditSvc *services.AuditService) fiber.Handler {
	return func(c *fiber.Ctx) error {
		limit := c.QueryInt("limit", 50)
		if limit <= 0 {
			limit = 50
		...
		logs, total, err := auditSvc.ListSuperAdminLogs(c.Context(), services.AuditLogFilters{
			Action:   action,
			Query:    query,
			TenantID: tenantID,
			Limit:    limit,
		...
		if err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"error": "Failed to list super admin audit logs",
			})
		}
